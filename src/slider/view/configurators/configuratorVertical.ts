    ...
    setInPlaceThumb(elements: HTMLElement[], modelState: IModelState, elementSliderLineSpan: HTMLElement, elementSliderLine: HTMLElement): void {
        new Array(elements.length)
            ...
            .forEach((_element: number, i: number) => {
                elements[i].style.top = String((Math.ceil(configuratorVertical.calculateCoefficientPoint(elementSliderLine, modelState.max, modelState.min) * modelState.thumbsValues[i]))) + 'px';
            })

        elementSliderLineSpan.style.marginTop = String(configuratorVertical.getElementOffset(elements[0])) + 'px';
        elementSliderLineSpan.style.height = String((configuratorVertical.getElementOffset(elements[elements.length - 1]) - configuratorVertical.getElementOffset(elements[0]))) + 'px';
    },
