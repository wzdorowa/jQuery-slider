...
2. В свою очередь модель подписывается, например, на изменение значения бегунка, которое регулирует вид. Вид оповещает об изменении значения бегунка. Модель получает данные об изменении значения бегунка, определенным образом обрабатывает полученные данные и оповещает об изменении своего состояния. Затем повторяется шаг 1.